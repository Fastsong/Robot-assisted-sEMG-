...
def remove_empty(arr):
    if isinstance(arr, list):
        return [remove_empty(x) for x in arr if x != []]
    else:
        return arr
...
for i in range(1,7):
    ...
    for k in range(14):
        ...
        file_name = f'D:\SEMG\sEMGdata\data\split\DX0\{k+1}.mat'
        mat_data = scio.loadmat(file_name)
        ...
        for j in range(len(data_label)):
            if data_label[j][0][2] == f'{i+9}         ' and data_label[j][0][0] != []:
                # if data_label[j][0][0] == []:
                #     label9fenlei_dict = {}
                # else:
                data_dict[f'Data{i}'].append(data_data[j])
                
            # elif i==2 and data_label[j][0][2] == '2          ':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==3 and data_label[j][0][2] == '3          ':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==4 and data_label[j][0][2] == '4          ':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==5 and data_label[j][0][2] == '5          ':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==6 and data_label[j][0][2] == '6          ':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==7 and data_label[j][0][2] == '7':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==8 and data_label[j][0][2] == '8':
            #     data_dict[f'Data{i}'].append(data_data[j])
            # elif i==9 and data_label[j][0][2] == '9':
            #     data_dict[f'Data{i}'].append(data_data[j])
        


        # data_dict[f'Data{i}'] = data_data
        data_dict[f'Label{i}'] = 0
...
for i in range(1,7):
    ...
    label_lin1 = np.zeros((processed_data_all.shape[0], 1))
    ...
    scio.savemat(f'D:\SEMG\data\prodata\ML2\{i}\DX0.mat',
