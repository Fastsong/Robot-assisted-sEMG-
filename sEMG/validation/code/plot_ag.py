...
# 初始化模型列表
models = [load(f'ag_model_subject{i}_Random Forest.joblib') for i in range(1, 10)]  # 加载9个模型

# 初始化存储所有样本结果的字典
all_Movements = {f'Week {i-1}': {j: [] for j in range(1, 10)} for i in range(1, 4)}  # 3个样本，每个样本9个动作

# 遍历三个样本数据
for i in range(1, 4):
    new_data_path = fr'D:\SEMG\data\prodata\ML6\2-{i}.mat'
    ...
    for k in range(len(X)):
        ...
        if label_str.isdigit():
            label = int(label_str)
            if 1 <= label <= 9:
                all_Movements[f'Week {i-1}'][label].append(y_preds[label - 1][k])  # 将对应模型的预测结果加入
...
Movement_list = [f'Movement{i}' for i in range(1, 10)]
